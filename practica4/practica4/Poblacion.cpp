	...
	crucesEsperados(0)
{
	...
	crucesEsperados = (unsigned int) _probabilidadCruce * (_tamPoblacion / 2);
	mutacionesEsperadas = (unsigned int) _probabilidadMutacion * _tamPoblacion * _tamIndividuo;
...
void Poblacion::cruzar() {
	...
	unsigned int numPadres = crucesEsperados*2;
...
void Poblacion::crucePMX(Solucion* padre1,Solucion* padre2,Solucion* hijo_1,Solucion* hijo_2){
		...
		unsigned int corte1 = rand()%tamIndividuo;
		unsigned int corte2 = rand()%tamIndividuo;
		if(corte1 > corte2){
			int c = corte1;
			corte1 = corte2;
			corte2 = c;
...
/**
 * Aplica la busqueda local a la poblacion por cromosoma (miembro)
 * @param probabilidadCromosoma Probabilidad de aplicar la busqueda a cada miembro
 * @param buscarMejores Indicar a true si solo se quiere aplicar sobre los mejores individuos
 */
void Poblacion::busquedaLocal(double probabilidadCromosoma, bool buscarMejores) {
	if (!buscarMejores) {
		for (unsigned int i = 0; i < individuos.size(); i++) {
			double _rand = ((double) rand() / (RAND_MAX));
			if (_rand <= probabilidadCromosoma) {
				//cout << individuos[i] << ": " << individuos[i]->getValorSolucionActual() << endl;
				individuos[i]->busquedaLocal();
				//cout << individuos[i] << ": " << individuos[i]->getValorSolucionActual() << endl;
				//system("pause");
			}
		}
		
	// Obtenemos los n mejores elementos de la poblacion y
	// aplicamos la busqueda
	} else {
		unsigned int n = (unsigned int) (individuos.size() * probabilidadCromosoma);
		Solucion** mejores = new Solucion*[n];

		for (unsigned int i = 0; i < n; i++) {
			mejores[i] = individuos[i];
			for (unsigned int j = 0; j < individuos.size(); j++) {
				bool yaEscogido = false;
				for (unsigned int k = 0; k < i && !yaEscogido; k++) {
					if (individuos[j] == mejores[k]) {
						yaEscogido = true;
					}
				}
				if (!yaEscogido && (individuos[j]->getValorSolucionActual() < mejores[i]->getValorSolucionActual())) {
					mejores[i] = individuos[j];
				}
			}

			//cout << mejores[i] << ": " << mejores[i]->getValorSolucionActual() << endl;
			mejores[i]->busquedaLocal();
			//cout << mejores[i] << ": " << mejores[i]->getValorSolucionActual() << endl;
			//system("pause");
		}

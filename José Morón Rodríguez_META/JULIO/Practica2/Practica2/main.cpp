...
void gerenacional();
void estacionario();
/*FUNCION PARA ORDENAR EL VECTOR DE LA POBLACION SEGUN EL VALOR DEL CROMOSOMA YA CALCULADO*/
...
int main(int argc, char* argv[]){
	...
	while (!salir){
		...
		if (!salir){
			...
			cout<<"1.- Genetico Generacional"<<endl;
			cout<<"2.- Genetico Estacionario "<<endl;
			cout<<"3.- Salir "<<endl;
			opcion=0;
			...
			switch (opcion){
			case 1:
				algoritmoElegido="Genetico Generacional";
				gerenacional();
				break;
			case 2:
				algoritmoElegido="Genetico Estacionario";
				estacionario();
				break;
			case 3:
				salir=true;
			...
			cout<<"Solucion Final: "<<poblacion[0].second<<endl;
...
void generarPopInicial(){
	...
	//CREAMOS UN CROMOSOMA DE 0 AL NUMERO DE PATRONES -1 PARA LUEGO MEZCARLO
	for (int i=0; i<elementos.size();i++) cromosoma_modelo.push_back(i);
	poblacion.clear();
	for(int i=0; i<50; i++){
		vector<int> cromosoma=cromosoma_modelo;
		random_shuffle(cromosoma.begin(), cromosoma.end());//MEZCLAMOS EL CROMOSOMA MODELO
		double J=funcFiness(cromosoma);//CALCULAMOS SU FUNCION OBJETIVO/FITNNES
...
double funcFiness(vector<int> cromosoma){
	...
	//INSERTAMOS LOS CENTROIDES EN EL VECTOR
...
void gerenacional(){
	generarPopInicial();//GENERAMOS LA POBLACION INICIAL
	...
	//VECTOR QUE GUARDA ENTEROS DESDE 0 HASTA EL NUMERO DE PATRONES-1, SERA DE DONDE SE SAQUEN QUE GENES MUTAR
	vector<int> posMutaciones;
	for (int i=0; i<elementos.size(); i++) posMutaciones.push_back(i);
	random_shuffle(posMutaciones.begin(), posMutaciones.end());//MEZCLAMOS LOS GENES
	int contPosMutaciones=0;
	while (contador<10000){
		sort(poblacion.begin(), poblacion.end(), orderPoblacion);
		if (poblacion[0].second<solucion){//GUARDAMOS LA MEJOR SOLUCION.
		...
		//GENERAMOS UN VECTOR DEL TAMAÑO DE LA POBLACION DE PADRES DEL CUAL IREMOS COJIENDO LA POSICION DE ESTOS.
		random_shuffle(posPadres.begin(), posPadres.end());
		vector<pair<vector<int>, double>> poblacionInter=poblacion;//LA POBLACION INTERMEDIA ES UNA COPIA DE LA POBLACION.
		for (int i=0; i<numCruces; i++){
			bool encontrado=false;
			//COJEMOS 4 POSICIONES ALEATORIAS PARA SELECCIONAR A LOS CANDIDATOS A PADRES
			...
			//SI EL SELECCIONADO ES EL MEJOR SE GUARDA PARA LUEGO MAS TARDE INSERTARLO
			if (elegido1==0&&!mejorMuerto){
				mejorMuerto=true;
			}
...
void estacionario(){
	...
	int contPosMutaciones=0;
	int contPosPadres=0;
	for (int i=0; i<50; i++)posPadres.push_back(i);
	random_shuffle(posPadres.begin(), posPadres.end());
	solucion=99999999;
	while (contador<10000){
		...
		vector<pair<vector<int>, double>> poblacionInter;
		int aleatorio1=posPadres[contPosPadres++];
		int aleatorio2=posPadres[contPosPadres++];
		int aleatorio3=posPadres[contPosPadres++];
		int aleatorio4=posPadres[contPosPadres++];
		//SI LOS CUATRO SIGUENTES SE PASA DE TAMAÑAO
		if (contPosPadres+4>=posPadres.size()-1){
			posPadres.clear();
			for (int i=0; i<50; i++)posPadres.push_back(i);
			random_shuffle(posPadres.begin(), posPadres.end());
			contPosPadres=0;
		...
		//TORNEO BINARIO ESCOJEMOS LOS QUE MEJOR SOLUCION TIENEN.
		vector<int> padre1=poblacion[aleatorio1].first;
		int elegido1=aleatorio1;
		if (poblacion[aleatorio2].second<poblacion[aleatorio1].second){
			padre1=poblacion[aleatorio2].first;
			elegido1=aleatorio2;
		}
			
		vector<int> padre2=poblacion[aleatorio3].first;
		int elegido2=aleatorio3;
		if (poblacion[aleatorio4].second<poblacion[aleatorio3].second){
			padre2=poblacion[aleatorio4].first;
			elegido2=aleatorio4;
		}
		...
		//}
		//MUTAMOS LA POBLACION INTERMEDIA
		int numMutacions=(int)(2*numGenes*probM);
		...
		//INSERTAMOS EN LA POBLACION LOS DOS HIJOS NUEVOS, ORDENAMOS Y BORRAMOS LOS DOS ULTIMOS.
		poblacion.push_back(poblacionInter[0]);
		poblacion.push_back(poblacionInter[1]);
		sort(poblacionInter.begin(), poblacionInter.end(), orderPoblacion);
